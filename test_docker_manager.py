...
class TestDockerManager(unittest.TestCase):
    ...
    def test_ssh_command_executor(self):
        executor = SSHCommandExecutor('test_host', 22, 'test_user', 'test_password', connect=False)
        with patch('paramiko.SSHClient.connect') as mock_connect:
            with patch('paramiko.SSHClient.exec_command') as mock_exec:
                ...
                executor.connect()
                mock_connect.assert_called_once_with('test_host', port=22, username='test_user', password='test_password')
    ...
    def test_local_command_executor(self, mock_run):
        ...
        output = executor.run_command('test_command')
        mock_run.assert_called_once_with('test_command', shell=True, text=True, capture_output=True)
        self.assertEqual(output, "test_output")
